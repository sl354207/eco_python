...
wwf_animals = pd.merge(tester, wwf_animals, how="inner", on=["scientific_name"])
...
inner_animal.rename(
    columns={"family_x": "family", "class_x": "class", "order_x": "order"}, inplace=True
)
...
inner_animal["combined"] = inner_animal["unique_id_x"].apply(
    lambda x: x.tolist()
) + inner_animal["unique_id_y"].apply(lambda x: x.tolist())
...
df = pd.merge(tester, inner_plant, how="outer", on=["scientific_name"], indicator=True)
...
# %%
df3 = pd.merge(df1, inner_animal, how="outer", on=["scientific_name"], indicator=True)
...
df4 = pd.merge(
    us_plants, inner_plant, how="outer", on=["scientific_name"], indicator=True
)

us_filter = df4.loc[
    df4._merge == "left_only",
    [
        "kingdom",
        "phylum",
        "class",
...
# %%
df5 = pd.merge(
    wwf_animals, inner_animal, how="outer", on=["scientific_name"], indicator=True
)

wwf_filter = df5.loc[
    df5._merge == "left_only",
    [
        "kingdom",
        "phylum",
        "class_x",
...
inner_plant = inner_plant.drop(
    ["family_y", "phylum_y", "class_y", "order_y", "rights_y", "genus_y", "kingdom_y"],
    axis=1,
)
...
inner_plant["combined"] = inner_plant["unique_id_x"] + inner_plant["unique_id_y"].apply(
    lambda x: x.tolist()
)
...
# %%
df3 = pd.merge(
    species, inner_plant, how="outer", on=["scientific_name"], indicator=True
)
...
for j in range(len(df)):
    df[j] = [i for n, i in enumerate(df[j]) if i not in df[j][n + 1 :]]
...
path_to_json = (
    "/media/muskrat/060A9B8E0A9B78FF/eco_data/main_eco_data/final/common_names"
)
